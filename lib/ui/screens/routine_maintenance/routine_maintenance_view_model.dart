...
class RoutineMaintenanceViewModel extends ChangeNotifier {
  ...
  bool _isRoutineMaintenanceUpdating = false;

  bool get isDataLoading => _isDataLoading;
  bool get isRoutineMaintenanceUpdating => _isRoutineMaintenanceUpdating;
  ...
  RoutineMaintenanceViewModel() {
    getRoutineMaintenanceList();
  }

  Future<void> getRoutineMaintenanceList() async {
    _isDataLoading = true;
    notifyListeners();

    final routineMaintenanceList =
        await _routineMaintenanceService.getRoutineMaintenanceList();
  ...
  Future<void> resetEngineHoursValue({
    ...
    _isRoutineMaintenanceUpdating = true;
    notifyListeners();

    await _routineMaintenanceService.updateRoutineMaintenance(
      objectId: objectId,
      engineHoursValue: 0,
    );

    final updatedRoutineMaintenance = await _routineMaintenanceService.getRoutineMaintenance(objectId: objectId);
    _vehicleNodeDataList[vehicleNodeDataIndex].routineMaintenanceList[routineMaintenanceIndex] = updatedRoutineMaintenance;

    _isRoutineMaintenanceUpdating = false;
    notifyListeners();
  }
